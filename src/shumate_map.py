"""A real, zoomable OpenStreetMap view built on libshumate.

libshumate ships in the GNOME runtime, so this needs no bundled dependency —
only network access for the tiles (declared in the Flatpak manifest). It is
imported defensively by window.py: if libshumate is somehow missing, or the
map fails to build, Easel falls back to the offline dotted map, so the app is
never left without a Map view.

The widget exposes the same tiny interface as the offline MapView
(`set_photos` / `set_activate_cb`) so the two are interchangeable. Photos taken
at essentially the same spot (~100 m) share one pin that shows a count;
clicking a pin opens all of its photos in the lightbox.
"""
import gi
...
class ShumateMap(Gtk.Box):
    ...
    with one marker per photo location."""
    ...
    def set_photos(self, entries):
        ...
        # Group photos taken at essentially the same place (rounding to 3
        # decimals ≈ 110 m) so one spot is one pin.
        groups = {}
    ...
    def _make_marker(self, lat, lon, photos):
        ...
        n = len(photos)
        if n > 1:
            pin.append(Gtk.Label(label=str(n) if n < 1000 else "999+",
                                 css_classes=["map-pin-count"]))
        else:
            pin.add_css_class("map-pin-single")
